...
//
//    private val viewModel by viewModels<SeatsVM>()
...
//                binding?.totalPrice?.text = getString(R.string.total_price, totalPrice)
                ...
                viewModelSeat.updateSelectedSeats(selectedSeat)
                viewModelSeat.getTotalPrice(totalPrice)
            }
        ...
        viewModelSeat.daysList.observe(viewLifecycleOwner) { days ->
            adapterDate.updateList(days)
        }

        viewModelSeat.timeSlots.observe(viewLifecycleOwner) { time ->
        ...
        binding?.buyTicket?.setOnClickListener {
            ...
            val selectedDate = viewModelSeat.selectedDate.value
            val selectedTime = viewModelSeat.selectedTime.value
            val selectedSeats = viewModelSeat.selectedSeats.value
